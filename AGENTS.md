...
This repository uses **Fastify + TypeScript (strict) + Kysely (PostgreSQL) + Dbmate migrations + pg-boss + TypeBox schemas**.
...
- Migrations: **Dbmate** SQL migrations in `src/database/migrations`
- Queue: **pg-boss**
...
- Migration files live in `src/database/migrations` and use Dbmate SQL format with `-- migrate:up` and `-- migrate:down` sections.
- Prefer timestamp-style Dbmate filenames generated by `dbmate new`, e.g. `20260621110100_create_users.sql`.
- Do not add Kysely TypeScript migrations or another migration runner.
- No schema drift: update SQL migrations and regenerate `src/database/db.d.ts` together via `make db-types` or the migration Make targets.
