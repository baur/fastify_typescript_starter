...
- Kysely for typed query building and Dbmate for migrations
...
Database types, Dbmate migration files, and shared query helpers live here. This folder is intentionally explicit because schema evolution is one of the most important parts of a backend codebase.
...
2. generating JWT signing keys
3. installing project dependencies
4. installing Git hooks through Lefthook
5. building and starting the local containers
...
Windows 11 users can run the project through the included Dev Container instead of installing Unix-like tooling for the Makefile.
...
Open the repository in VS Code and choose `Dev Containers: Reopen in Container`. The container setup creates `.env` from `.env.example` when needed, generates local JWT values, installs dependencies, and starts PostgreSQL plus Mailpit.
